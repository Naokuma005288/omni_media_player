    ...
    try {
      ...
      btn.onclick = async () => {
        try {
          ...
          if (!src) { (runtime?.toast || window.toast)?.('メディアがありません','warn'); return; }

          // 埋め込み(YouTube/iframe)は対象外
          if (pageState?.usingYouTube || pageState?.usingIframe) {
            (runtime?.toast || window.toast)?.('埋め込みソースのCastは未対応','warn'); return;
          }
          ...
          session.loadMedia(req).then(
            () => (runtime?.toast || window.toast)?.('Casting開始'),
            (e) => (runtime?.toast || window.toast)?.('Cast失敗: '+(e?.code||''), 'err')
          );
        } catch (e) {
          (runtime?.toast || window.toast)?.('Cast接続に失敗', 'err');
        }
      };
    } catch (e) {
