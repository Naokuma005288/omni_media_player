...
const themeKey='om.theme', settingsKey='om.settings', favKey='om.favs', accentKey='om.accent', orderKey='om.order', releaseDismissKey='om.release.dismiss.v4.7.0';
...
const dict={
  ja:{lang:"JP",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",landingTitle:"OmniMedia – ランディング",alpha:"推奨",beta:"mini",old:"非推奨",
      ...
      releaseTitle:"v4.7.0 の強化ポイント",
      releaseLead:"通常版の拍解析まわりを見直し、beat grid とメトロノームを実用寄りに強化しました。",
      releasePoint1:"BPM 解析は `DJ 用 BPM` と `meter 用 BPM` を分け、拍アンカーを持つ構成へ更新しました。",
      releasePoint2:"通常版の `status HUD` に beat grid を追加し、現在の `bar / beat / grid confidence` を見ながら確認できます。",
      releasePoint3:"メトロノームは解析済み BPM と拍アンカーを共通参照するようにし、以前より拍位置が合いやすくなりました。",
      releasePoint4:"メトロノームへ `4分 / 8分` 切替と視覚クリックを追加し、耳と HUD の両方で拍を追えるようにしました。",
  ...
  en:{lang:"EN",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",landingTitle:"OmniMedia – Landing",alpha:"Rec",beta:"mini",old:"Old",
      ...
      releaseTitle:"What is new in v4.7.0",
      releaseLead:"The normal player now tightens beat analysis and makes the beat grid and metronome more usable.",
      releasePoint1:"BPM analysis now separates `DJ BPM` from `meter BPM` and stores a beat anchor for timing-aware playback tools.",
      releasePoint2:"The normal player gets a beat grid on the `status HUD`, so you can see the current `bar / beat / grid confidence` at a glance.",
      releasePoint3:"The metronome now follows the analyzed BPM and shared beat anchor more closely, so clicks line up better than before.",
      releasePoint4:"New metronome options add `quarter / eighth` subdivision switching and visual clicks on the HUD.",
  ...
  ko:{lang:"KR",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"추천",beta:"mini",old:"비추천",
      ...
      releaseTitle:"v4.7.0 업데이트",
      releaseLead:"일반 버전의 박자 분석을 다듬고 beat grid 와 메트로놈을 더 실용적으로 강화했습니다.",
      releasePoint1:"BPM 분석은 `DJ BPM` 과 `meter BPM` 을 분리하고 beat anchor 를 함께 보관하도록 바뀌었습니다.",
      releasePoint2:"일반 버전의 `status HUD` 에 beat grid 가 추가되어 현재 `bar / beat / grid confidence` 를 볼 수 있습니다.",
      releasePoint3:"메트로놈은 분석된 BPM 과 공용 beat anchor 를 따라가도록 바뀌어 이전보다 클릭 위치가 더 잘 맞습니다.",
      releasePoint4:"메트로놈에 `4분 / 8분` 전환과 HUD 시각 클릭 옵션이 추가되었습니다.",
  ...
  zh:{lang:"中文",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"推荐",beta:"mini",old:"旧版",
      ...
      releaseTitle:"v4.7.0 更新内容",
      releaseLead:"普通版重新整理了节拍分析，并把 beat grid 与节拍器增强到更实用的状态。",
      releasePoint1:"BPM 分析现在会区分 `DJ BPM` 与 `meter BPM`，并保存 beat anchor 供节拍工具使用。",
      releasePoint2:"普通版的 `status HUD` 新增 beat grid，可直接查看当前 `bar / beat / grid confidence`。",
      releasePoint3:"节拍器会更紧密地跟随分析后的 BPM 与共享 beat anchor，点击位置比以前更容易对上。",
      releasePoint4:"节拍器新增 `4分 / 8分` 切换，以及 HUD 上的视觉点击提示。",
  ...
  ru:{lang:"RU",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"Реком.",beta:"mini",old:"Стар.",
      ...
      releaseTitle:"Что нового в v4.7.0",
      releaseLead:"Обычная версия усиливает анализ ритма и делает beat grid с метрономом заметно полезнее.",
      releasePoint1:"Анализ BPM теперь разделяет `DJ BPM` и `meter BPM`, а также хранит beat anchor для привязки по времени.",
      releasePoint2:"В обычной версии на `status HUD` появился beat grid, где видно текущие `bar / beat / grid confidence`.",
      releasePoint3:"Метроном теперь лучше следует за проанализированным BPM и общим beat anchor, поэтому клики попадают точнее.",
      releasePoint4:"Добавлены переключение `quarter / eighth` и визуальные клики метронома прямо на HUD.",
  ...
  fr:{lang:"FR",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"Recom.",beta:"mini",old:"Anc.",
      ...
      releaseTitle:"Nouveautés de v4.7.0",
      releaseLead:"La version standard renforce maintenant l’analyse du tempo et rend le beat grid comme le métronome plus exploitables.",
      releasePoint1:"L’analyse BPM sépare désormais `DJ BPM` et `meter BPM`, avec un beat anchor conservé pour les outils rythmiques.",
      releasePoint2:"La version standard ajoute un beat grid au `status HUD`, avec le `bar / beat / grid confidence` visible en un coup d’œil.",
      releasePoint3:"Le métronome suit plus précisément le BPM analysé et le beat anchor partagé, donc les clics tombent mieux qu’avant.",
      releasePoint4:"De nouvelles options ajoutent le choix `quarter / eighth` et des clics visuels directement sur le HUD.",
