...
const themeKey='om.theme', settingsKey='om.settings', favKey='om.favs', accentKey='om.accent', orderKey='om.order', releaseDismissKey='om.release.dismiss.v5.3.0';
...
const dict={
  ja:{lang:"JP",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",landingTitle:"OmniMedia – ランディング",alpha:"推奨",beta:"mini",old:"非推奨",
      ...
      releaseTitle:"v5.3.0 の更新内容",
      releaseLead:"v5.3.0 では通常版の映像フォーカスを mini 版に近い重ね表示 UI へ再設計し、黒帯なし・自動非表示の映像集中モードへ更新しました。",
      releasePoint1:"映像フォーカスの下バーは mini 版に近い 2 段 UI へ再構成し、映像の上に重なるオーバーレイ表示になりました。",
      releasePoint2:"コントロールバーはほぼ透明のガラス調になり、一定時間操作がないと自動で隠れます。",
      releasePoint3:"映像はフォーカス時に黒帯を出さない表示へ切り替わり、操作時だけバーを呼び戻せます。",
      releasePoint4:"service worker の precache 対象と cache version も更新し、landing / mini / legacy を含めて配布更新が反映されやすくなりました。",
  ...
  en:{lang:"EN",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",landingTitle:"OmniMedia – Landing",alpha:"Rec",beta:"mini",old:"Old",
      ...
      releaseTitle:"What is new in v5.3.0",
      releaseLead:"The v5.3.0 release rebuilds normal-player video focus into a mini-style overlay UI with no black bars and idle auto-hide.",
      releasePoint1:"Video focus now uses a mini-style two-row overlay bar that sits on top of the picture instead of reserving space below it.",
      releasePoint2:"The focus bar is now near-transparent glass and automatically hides after a short idle period while playback continues.",
      releasePoint3:"Focused video switches to a no-black-bar presentation, so the stage stays filled while controls appear only when needed.",
      releasePoint4:"The service worker cache version and precache list were updated so landing, mini, and legacy assets refresh more reliably.",
  ...
  ko:{lang:"KR",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"추천",beta:"mini",old:"비추천",
      ...
      releaseTitle:"v5.3.0 업데이트",
      releaseLead:"v5.3.0 은 일반 버전의 영상 포커스를 mini 스타일 오버레이 UI로 다시 설계하고, 검은 여백 없는 화면과 자동 숨김 동작을 추가한 릴리스입니다.",
      releasePoint1:"영상 포커스의 하단 바는 mini 버전에 가까운 2단 오버레이 UI로 바뀌어 영상 위에 겹쳐집니다.",
      releasePoint2:"컨트롤 바는 거의 투명한 유리 느낌으로 바뀌었고, 잠시 조작이 없으면 자동으로 숨겨집니다.",
      releasePoint3:"포커스 중인 영상은 검은 띠 없이 채워서 표시되며, 필요할 때만 컨트롤 바를 다시 불러올 수 있습니다.",
      releasePoint4:"service worker 의 cache version 과 precache 목록도 갱신되어 landing / mini / legacy 자산이 더 안정적으로 새로고침됩니다.",
  ...
  zh:{lang:"中文",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"推荐",beta:"mini",old:"旧版",
      ...
      releaseTitle:"v5.3.0 更新内容",
      releaseLead:"v5.3.0 重新设计了普通版的视频聚焦模式，改成接近 mini 版的叠加式 UI，并加入无黑边和自动隐藏控制栏。",
      releasePoint1:"视频聚焦现在使用接近 mini 版的双层叠加控制栏，直接浮在画面上，而不是继续占用下方空间。",
      releasePoint2:"控制栏改成近乎透明的玻璃感样式，在短时间无操作后会自动隐藏。",
      releasePoint3:"聚焦时的视频会以无黑边方式铺满显示，需要操作时再把控制栏唤回即可。",
      releasePoint4:"service worker 的 cache version 与 precache 列表也已更新，landing / mini / legacy 资源会更稳定地刷新。",
  ...
  ru:{lang:"RU",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"Реком.",beta:"mini",old:"Стар.",
      ...
      releaseTitle:"Что нового в v5.3.0",
      releaseLead:"В v5.3.0 режим video focus обычной версии переработан в overlay UI в стиле mini, без чёрных полос и с автоскрытием при бездействии.",
      releasePoint1:"Video focus теперь использует двухрядную overlay-панель в стиле mini, которая накладывается прямо на изображение.",
      releasePoint2:"Панель управления стала почти прозрачной, в стеклянном стиле, и автоматически скрывается после короткого периода без действий.",
      releasePoint3:"Видео в режиме focus теперь заполняет сцену без чёрных полос, а элементы управления возвращаются только при необходимости.",
      releasePoint4:"Также обновлены cache version и precache-список service worker, поэтому landing / mini / legacy активы обновляются надёжнее.",
  ...
  fr:{lang:"FR",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"Recom.",beta:"mini",old:"Anc.",
      ...
      releaseTitle:"Nouveautes de v5.3.0",
      releaseLead:"v5.3.0 revoit le mode focus video de la version standard avec une UI en surimpression proche de mini, sans bandes noires et avec masquage automatique.",
      releasePoint1:"Le focus video utilise maintenant une barre overlay a deux lignes, proche de mini, directement posee sur l'image.",
      releasePoint2:"La barre de controle devient presque transparente, avec un rendu verre, et se masque automatiquement apres une courte periode d'inactivite.",
      releasePoint3:"La video remplit la scene sans bandes noires en mode focus, et les commandes reviennent seulement quand il faut agir.",
      releasePoint4:"La cache version et la liste precache du service worker ont aussi ete mises a jour pour mieux rafraichir les assets landing / mini / legacy.",
