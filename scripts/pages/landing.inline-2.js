...
const themeKey='om.theme', settingsKey='om.settings', favKey='om.favs', accentKey='om.accent', orderKey='om.order', releaseDismissKey='om.release.dismiss.v4.8.0';
...
const dict={
  ja:{lang:"JP",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",landingTitle:"OmniMedia – ランディング",alpha:"推奨",beta:"mini",old:"非推奨",
      ...
      releaseTitle:"v4.8.0 の強化ポイント",
      releaseLead:"通常版の Auto DJ へ手動介入と次曲プレビューを追加し、候補の見え方と運用性を強化しました。",
      releasePoint1:"通常版に `NEXT` プレビューカードを追加し、次に選ぶ曲と判断材料を `AUTO / LOCK / LIVE` 付きで見えるようにしました。",
      releasePoint2:"プレイリスト各行から `次に回す曲` を直接固定できるようにし、Auto DJ の手動介入を追加しました。",
      releasePoint3:"`選び直し` と `解除` を追加し、候補の reroll と手動固定解除を通常版 UI から即座に行えるようにしました。",
      releasePoint4:"候補選択ロジックは preview / reroll 前提で再利用できるよう整理し、次曲の見通しを取りやすくしました。",
  ...
  en:{lang:"EN",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",landingTitle:"OmniMedia – Landing",alpha:"Rec",beta:"mini",old:"Old",
      ...
      releaseTitle:"What is new in v4.8.0",
      releaseLead:"The normal player now adds manual Auto DJ intervention and a visible next-track preview workflow.",
      releasePoint1:"A new `NEXT` preview card shows the upcoming Auto DJ choice and its reasoning with `AUTO / LOCK / LIVE` states.",
      releasePoint2:"Each playlist row can now lock a specific track as the next Auto DJ target, giving you direct manual control.",
      releasePoint3:"New `reroll` and `clear` actions let you swap the candidate or drop manual overrides straight from the main UI.",
      releasePoint4:"Candidate selection is now exposed through reusable preview logic, making the next-track flow easier to inspect.",
  ...
  ko:{lang:"KR",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"추천",beta:"mini",old:"비추천",
      ...
      releaseTitle:"v4.8.0 업데이트",
      releaseLead:"일반 버전에 Auto DJ 수동 개입과 다음 곡 프리뷰를 추가해 후보 확인과 운용성을 강화했습니다.",
      releasePoint1:"새 `NEXT` 프리뷰 카드가 다음 Auto DJ 곡과 판단 근거를 `AUTO / LOCK / LIVE` 상태로 보여줍니다.",
      releasePoint2:"플레이리스트 각 행에서 특정 곡을 다음 곡으로 바로 고정할 수 있어 수동 개입이 가능해졌습니다.",
      releasePoint3:"`선택 다시` 와 `해제` 를 추가해 후보 reroll 과 수동 고정을 메인 UI 에서 바로 처리할 수 있습니다.",
      releasePoint4:"후보 선택 로직을 preview / reroll 전제로 재사용하도록 정리해 다음 곡 흐름을 더 읽기 쉽게 만들었습니다.",
  ...
  zh:{lang:"中文",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"推荐",beta:"mini",old:"旧版",
      ...
      releaseTitle:"v4.8.0 更新内容",
      releaseLead:"普通版加入了 Auto DJ 手动介入与下一曲预览，让候选查看和实际操作更直接。",
      releasePoint1:"新增 `NEXT` 预览卡片，可用 `AUTO / LOCK / LIVE` 状态查看下一首候选和判断依据。",
      releasePoint2:"播放列表每一行都可以直接把某首歌固定为下一首，实现手动介入 Auto DJ。",
      releasePoint3:"新增 `重选` 与 `解除`，可在主界面直接 reroll 候选或取消手动固定。",
      releasePoint4:"候选选择逻辑已整理为可复用的 preview / reroll 流程，更容易理解下一首的决策。",
  ...
  ru:{lang:"RU",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"Реком.",beta:"mini",old:"Стар.",
      ...
      releaseTitle:"Что нового в v4.8.0",
      releaseLead:"Обычная версия получает ручное вмешательство в Auto DJ и более наглядный предпросмотр следующего трека.",
      releasePoint1:"Новая карточка `NEXT` показывает следующий выбор Auto DJ и причины выбора со статусами `AUTO / LOCK / LIVE`.",
      releasePoint2:"Каждую строку плейлиста теперь можно зафиксировать как следующий трек, добавляя ручное управление Auto DJ.",
      releasePoint3:"Кнопки `reroll` и `clear` позволяют сразу заменить кандидата или снять ручную фиксацию из основного UI.",
      releasePoint4:"Логика выбора кандидатов вынесена в общий preview / reroll поток, поэтому следующий трек легче понять и контролировать.",
  ...
  fr:{lang:"FR",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"Recom.",beta:"mini",old:"Anc.",
      ...
      releaseTitle:"Nouveautés de v4.8.0",
      releaseLead:"La version standard ajoute maintenant une intervention manuelle pour Auto DJ et un aperçu plus clair du morceau suivant.",
      releasePoint1:"Une nouvelle carte `NEXT` affiche le prochain choix Auto DJ et ses raisons avec les états `AUTO / LOCK / LIVE`.",
      releasePoint2:"Chaque ligne de playlist peut désormais verrouiller un morceau précis comme prochain titre Auto DJ.",
      releasePoint3:"Les actions `reroll` et `clear` permettent de changer le candidat ou de retirer le verrouillage manuel depuis l’UI principale.",
      releasePoint4:"La logique de sélection des candidats est réutilisée pour le preview et le reroll, ce qui rend le flux du prochain titre plus lisible.",
