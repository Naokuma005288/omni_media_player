...
const themeKey='om.theme', settingsKey='om.settings', favKey='om.favs', accentKey='om.accent', orderKey='om.order', releaseDismissKey='om.release.dismiss.v4.6.0';
...
const dict={
  ja:{lang:"JP",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",landingTitle:"OmniMedia – ランディング",alpha:"推奨",beta:"mini",old:"非推奨",
      ...
      releaseTitle:"v4.6.0 の強化ポイント",
      releaseLead:"通常版の Auto DJ を土台から強化し、遷移・選曲・失敗復帰・設定周りまで見直しました。",
      releasePoint1:"cue-in、小節合わせ、ラウドネス差、intro / outro 傾向を見て、遷移プロファイルを自動で切り替えるようにしました。",
      releasePoint2:"前方候補から次曲を選び、BPM・音量差・動画優先度・最近使った曲の回避を踏まえて Auto DJ できるようにしました。",
      releasePoint3:"`mp4 -> mp4` を含む handoff の監視を強化し、起動失敗候補のクールダウンと上限リトライを追加しました。",
      releasePoint4:"Auto DJ モード、候補先読み数、再登場回避の設定を追加し、動作中の phase も status へ出すようにしました。",
  ...
  en:{lang:"EN",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",landingTitle:"OmniMedia – Landing",alpha:"Rec",beta:"mini",old:"Old",
      ...
      releaseTitle:"What is new in v4.6.0",
      releaseLead:"The normal player now upgrades Auto DJ across transition quality, candidate selection, failure recovery, and control settings.",
      releasePoint1:"Auto DJ now reads cue-in, bar alignment, loudness gaps, and intro/outro character to switch transition profiles automatically.",
      releasePoint2:"It can choose the next track from forward candidates using BPM, loudness, media type, and recent-play avoidance instead of only taking the next index.",
      releasePoint3:"`mp4 -> mp4` handoff is more resilient with candidate cooldowns, bounded retry cycles, and cleaner fallback behavior.",
      releasePoint4:"New Auto DJ controls let you pick a mode, adjust candidate lookahead, and toggle repeat avoidance while phase/status stays visible.",
  ...
  ko:{lang:"KR",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"추천",beta:"mini",old:"비추천",
      ...
      releaseTitle:"v4.6.0 업데이트",
      releaseLead:"일반 버전의 Auto DJ를 더 강하게 다듬어 전환 품질, 곡 선택, 실패 복구, 설정까지 확장했습니다.",
      releasePoint1:"cue-in, 마디 정렬, 음량 차이, intro / outro 성향을 보고 전환 프로필을 자동으로 바꿉니다.",
      releasePoint2:"앞쪽 후보 곡들 중에서 BPM, 음량, 미디어 종류, 최근 재생 회피를 반영해 다음 곡을 고를 수 있습니다.",
      releasePoint3:"`mp4 -> mp4` handoff 는 실패 후보 쿨다운과 제한된 재시도로 더 안정적으로 동작합니다.",
      releasePoint4:"Auto DJ 모드, 후보 선탐색 수, 반복 회피 설정이 추가되고 phase 상태도 표시됩니다.",
  ...
  zh:{lang:"中文",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"推荐",beta:"mini",old:"旧版",
      ...
      releaseTitle:"v4.6.0 更新内容",
      releaseLead:"普通版进一步强化了 Auto DJ，把过渡质量、候选选曲、失败恢复和设置项一起补强。",
      releasePoint1:"现在会参考 cue-in、小节对齐、音量差和 intro / outro 特征，自动切换不同的过渡配置。",
      releasePoint2:"Auto DJ 不再只看下一个索引，而是能从前方候选中按 BPM、音量、媒体类型和最近使用回避来选曲。",
      releasePoint3:"`mp4 -> mp4` 交接增加了失败候选冷却和有限重试，异常时也更容易平稳落地。",
      releasePoint4:"新增 Auto DJ 模式、候选先读数量和重复回避设置，并把 phase 状态显示出来。",
  ...
  ru:{lang:"RU",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"Реком.",beta:"mini",old:"Стар.",
      ...
      releaseTitle:"Что нового в v4.6.0",
      releaseLead:"Обычная версия усиливает Auto DJ: теперь улучшены переходы, выбор следующего трека, восстановление после сбоев и настройки.",
      releasePoint1:"Auto DJ учитывает cue-in, выравнивание по тактам, разницу громкости и характер intro / outro, чтобы выбирать профиль перехода.",
      releasePoint2:"Следующий трек может выбираться из нескольких кандидатов вперёд по BPM, громкости, типу медиа и защите от повторов.",
      releasePoint3:"`mp4 -> mp4` handoff стал устойчивее благодаря охлаждению неудачных кандидатов и ограниченным циклам повторной попытки.",
      releasePoint4:"Добавлены режимы Auto DJ, глубина просмотра кандидатов и переключатель защиты от повторов, а phase/status теперь виден в UI.",
  ...
  fr:{lang:"FR",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"Recom.",beta:"mini",old:"Anc.",
      ...
      releaseTitle:"Nouveautés de v4.6.0",
      releaseLead:"La version standard renforce Auto DJ sur la qualité des transitions, le choix du morceau suivant, la reprise en cas d’échec et les réglages.",
      releasePoint1:"Auto DJ lit désormais le cue-in, l’alignement par mesure, les écarts de volume et le caractère intro / outro pour choisir un profil de transition.",
      releasePoint2:"Le morceau suivant peut être choisi parmi plusieurs candidats en fonction du BPM, du volume, du type de média et de l’évitement des répétitions.",
      releasePoint3:"Le handoff `mp4 -> mp4` devient plus robuste avec un cooldown des candidats en échec et un nombre limité de relances.",
      releasePoint4:"De nouveaux réglages Auto DJ ajoutent le mode, la profondeur de présélection et l’évitement des répétitions, avec un status/phase visible.",
