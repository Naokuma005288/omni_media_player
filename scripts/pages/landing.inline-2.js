...
const themeKey='om.theme', settingsKey='om.settings', favKey='om.favs', accentKey='om.accent', orderKey='om.order', releaseDismissKey='om.release.dismiss.v5.0.0';
...
const dict={
  ja:{lang:"JP",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",landingTitle:"OmniMedia – ランディング",alpha:"推奨",beta:"mini",old:"非推奨",
      ...
      releaseTitle:"v5.0.0 の強化ポイント",
      releaseLead:"通常版の Auto DJ は、phrase / energy 解析を使って遷移タイミングと画面演出まで制御する構成へ進みました。",
      releasePoint1:"通常版は `intro / outro bars` と `entry / exit energy` を解析し、各曲ごとの mix しやすさを持つようになりました。",
      releasePoint2:"候補選択と sync profile は `phrase / energy` を参照し、次曲の選び方と `prime / launch / fade` を自動調整します。",
      releasePoint3:"bar 合わせは `meter BPM + beat anchor` 基準へ寄り、Auto DJ の起動位置と遷移窓も phrase overlap に応じて伸縮します。",
      releasePoint4:"`DJ Transition HUD` は `Bars / Energy / Fade` を表示し、画面全体にも `DJ transition accent` を追加しました。",
  ...
  en:{lang:"EN",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",landingTitle:"OmniMedia – Landing",alpha:"Rec",beta:"mini",old:"Old",
      ...
      releaseTitle:"What is new in v5.0.0",
      releaseLead:"The normal player now drives Auto DJ with phrase and energy analysis, adaptive transition timing, and stage-wide transition feedback.",
      releasePoint1:"Auto DJ now stores `intro / outro bars` and `entry / exit energy`, giving each track its own mix profile.",
      releasePoint2:"Candidate scoring and sync profile selection now use `phrase / energy`, not just BPM and loudness.",
      releasePoint3:"Beat and bar alignment now follows `meter BPM + beat anchors`, and the transition window can stretch with phrase overlap.",
      releasePoint4:"The `DJ Transition HUD` now exposes `Bars / Energy / Fade`, and a new `DJ transition accent` sweeps across the stage during mix and handoff.",
  ...
  ko:{lang:"KR",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"추천",beta:"mini",old:"비추천",
      ...
      releaseTitle:"v5.0.0 업데이트",
      releaseLead:"일반 버전 Auto DJ 는 phrase / energy 분석을 바탕으로 전환 타이밍과 화면 연출까지 제어하는 단계로 올라갔습니다.",
      releasePoint1:"각 곡에 `intro / outro bars` 와 `entry / exit energy` 를 저장해 곡별 mix 성향을 보존합니다.",
      releasePoint2:"후보 선택과 sync profile 은 이제 BPM 과 loudness 뿐 아니라 `phrase / energy` 도 함께 사용합니다.",
      releasePoint3:"bar 정렬은 `meter BPM + beat anchor` 기준으로 맞춰지고, phrase overlap 에 따라 transition window 도 늘어나거나 줄어듭니다.",
      releasePoint4:"`DJ Transition HUD` 는 `Bars / Energy / Fade` 를 보여주고, mix 와 handoff 중에는 새 `DJ transition accent` 가 무대 전체에 나타납니다.",
  ...
  zh:{lang:"中文",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"推荐",beta:"mini",old:"旧版",
      ...
      releaseTitle:"v5.0.0 更新内容",
      releaseLead:"普通版 Auto DJ 现在会使用 phrase / energy 分析来控制过渡时机，并把这些信息直接反馈到界面上。",
      releasePoint1:"每首曲目都会记录 `intro / outro bars` 与 `entry / exit energy`，形成自己的 mix 特征。",
      releasePoint2:"候选选择与 sync profile 不再只看 BPM 和 loudness，也会参考 `phrase / energy`。",
      releasePoint3:"bar 对齐改为基于 `meter BPM + beat anchor`，过渡窗口也会随 phrase overlap 自动伸缩。",
      releasePoint4:"`DJ Transition HUD` 现在会显示 `Bars / Energy / Fade`，并新增与 mix / handoff 同步的 `DJ transition accent`。",
  ...
  ru:{lang:"RU",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"Реком.",beta:"mini",old:"Стар.",
      ...
      releaseTitle:"Что нового в v5.0.0",
      releaseLead:"В обычной версии Auto DJ теперь использует анализ phrase / energy и управляет не только переходом, но и его визуальной подачей.",
      releasePoint1:"Для каждого трека сохраняются `intro / outro bars` и `entry / exit energy`, то есть собственный mix-профиль.",
      releasePoint2:"Выбор кандидата и sync profile теперь опираются не только на BPM и loudness, но и на `phrase / energy`.",
      releasePoint3:"Выравнивание по bar теперь строится на `meter BPM + beat anchor`, а окно перехода может растягиваться по overlap phrase.",
      releasePoint4:"`DJ Transition HUD` теперь показывает `Bars / Energy / Fade`, а новый `DJ transition accent` проходит по всей сцене во время mix и handoff.",
  ...
  fr:{lang:"FR",footer:"© 2025 OmniMedia – Built with ChatGPT-4o + ChatGPT-5.4",alpha:"Recom.",beta:"mini",old:"Anc.",
      ...
      releaseTitle:"Nouveautés de v5.0.0",
      releaseLead:"La version standard fait maintenant tourner Auto DJ avec une analyse phrase / energy, un timing de transition adaptatif et un retour visuel sur toute la scène.",
      releasePoint1:"Chaque morceau conserve désormais ses `intro / outro bars` et son `entry / exit energy`, soit son propre profil de mix.",
      releasePoint2:"Le choix du candidat et du sync profile utilise maintenant `phrase / energy`, pas seulement le BPM et la loudness.",
      releasePoint3:"L’alignement des bars suit `meter BPM + beat anchor`, et la fenêtre de transition peut s’étirer selon le chevauchement de phrase.",
      releasePoint4:"Le `DJ Transition HUD` affiche maintenant `Bars / Energy / Fade`, et un nouveau `DJ transition accent` traverse la scène pendant le mix et le handoff.",
