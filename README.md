...
### v5.3.0
- 通常版の `映像フォーカス` は、mini 版に寄せた 2 段オーバーレイ UI へ再設計しました。
- フォーカス用コントロールバーは映像に重なる半透明寄りのガラス UI になり、無操作時は自動で非表示になります。
- フォーカス時の映像は黒帯を出さない表示へ切り替わり、必要な時だけコントロールを呼び戻せます。
- `sw.js` の cache version と precache 対象を更新し、landing / mini / legacy を含めて更新反映を強めました。

### v5.2.0
- 通常版に `映像フォーカス` を追加し、左カラムと上バーだけを隠して映像へ集中できるようにしました。
- 映像フォーカス中も下のコントロールバーは残し、映像連動アンビエントライトなどの映像演出は維持されます。
- landing から開ける `README` ページを追加し、`README.md` をブラウザ上でそのまま読めるようにしました。
- README ページは見出しベースの目次と `Raw README.md` への導線を持ち、読み込みできない環境では埋め込みコピーへフォールバックします。

### v5.1.0
- 通常版の Auto DJ 更新で `ReferenceError` が出続ける問題を修正しました。
- 通常版に `manual BPM tap` を追加し、再生中に BPM 表示を左クリックして数回タップすると手動で BPM を合わせられるようにしました。
- service worker の precache と fetch 戦略を見直し、古い JS を掴み続ける問題と install 失敗に強くしました。
- mini はスマホ縦持ち時の seek レイアウトを修正し、シークバーが不自然に巨大化しないようにしました。
...
- プレイリスト各行から `次に回す曲` を直接固定できるようにし、Auto DJ の手動介入を追加しました。
- `選び直し` と `解除` を追加し、候補の reroll と手動固定解除を通常版 UI から即座に行えるようにしました。
- Auto DJ の候補選択は preview / reroll 前提で再利用できるよう整理し、通常版の運用性を上げました。

### v4.7.0
- 通常版の BPM 解析は `DJ 用 BPM` と `meter 用 BPM` を分離し、拍アンカーを持つ構成へ更新しました。
- メトロノームは解析済み BPM と拍アンカーを共通参照するようにし、以前より拍位置が合いやすくなりました。
- 通常版の `status HUD` に beat grid を追加し、現在の `bar / beat / grid confidence` を見ながら確認できるようにしました。
- メトロノームへ `4分 / 8分` 切替と視覚クリックを追加し、耳だけでなく HUD でも拍を追えるようにしました。

### v4.6.0
- 通常版の Auto DJ は cue-in、ラウドネス、intro / outro の静かさを見ながら遷移プロファイルを切り替える構成へ強化しました。
- beatSync 有効時は拍合わせから小節合わせへ寄せ、deck 起動と開始位置をより自然に合わせるよう改善しました。
- Auto DJ は前方候補から次曲を選べるようになり、BPM / 音量差 / イントロ・アウトロ傾向 / 動画優先度を見て選曲します。
- 起動失敗候補のクールダウン、handoff の上限リトライ、進行 phase 表示、Auto DJ モード設定を追加しました。
