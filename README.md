...
### v5.2.1
- 通常版の `映像フォーカス` は、映像ステージと操作パネルが中央にまとまるレイアウトへ調整しました。
- 下のコントロールバーはフォーカス専用に再設計し、再生・補助操作・スライダー群をコンパクトで使いやすい構成へ刷新しました。
- 下バー自体は残り、映像連動アンビエントライトなどの既存演出もそのまま維持されます。
- 縦長や狭幅ではフォーカス用コントロールバーが自動で 1 カラムへ切り替わります。
